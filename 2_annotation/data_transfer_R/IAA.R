...
#-----------------------------------------------AIOD-3LIB----------------------------------------------------
final_ad <- ad$read_h5ad("./data_transfer_R/output_data/IAA/AIOD_3LIB-pre_annt.h5ad")
...
saveRDS(object = seurat_obj, file = "./data_transfer_R/output_data/IAA/AIOD_3LIB-pre_annt.rds")

#-----------------------------------------------IA-1-3LIB----------------------------------------------------
final_ad <- ad$read_h5ad("./data_transfer_R/output_data/IAA/IA_1_3LIB-pre_annt.h5ad")
counts_matrix <- t(final_ad$X)
colnames(counts_matrix) <- final_ad$obs_names 
rownames(counts_matrix) <- final_ad$var_names  
seurat_obj <- Seurat::CreateSeuratObject(counts = counts_matrix, assay = "RNA",meta.data = final_ad$obs )
print(head(rownames(seurat_obj), 10))
print(head(colnames(seurat_obj), 10))
saveRDS(object = seurat_obj, file = "./data_transfer_R/output_data/IAA/IA_1_3LIB-pre_annt.rds")