...
rds_file <- "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0603_no_Basophil_state_score/mouse_plaque_type_pred_ml_marker_score_no_threshold_var.rds"
...
gene_map <- data.frame(
  feature_id = rownames(seu[["RNA"]]),
  original_gene_names = gene_meta$original_gene_names,
...
gene_map <- gene_map %>%
  dplyr::mutate(
    ...
    original_gene_names = as.character(original_gene_names),
    gene_symbol = ifelse(
...
seu <- subset(
  seu,
  subset = Plaque_type_pred %in% c("stable", "unstable")
)
...
# =========================
# 检查 gene_map 是否能和 pseudobulk count matrix 对上
# =========================

cat("Matched genes:", sum(rownames(pb_counts) %in% gene_map$feature_id), "\n")
cat("Total pb genes:", nrow(pb_counts), "\n")
cat("Match rate:", sum(rownames(pb_counts) %in% gene_map$feature_id) / nrow(pb_counts), "\n")

print(head(rownames(pb_counts)))
print(head(gene_map$feature_id))
print(head(gene_map$original_gene_names))
##########################
...
res <- results(
  dds,
  contrast = c("Plaque_type_pred", "unstable", "stable")
)
...
res <- res %>%
  ...
  dplyr::relocate(gene_symbol, original_gene_names, feature_id) %>%
  dplyr::arrange(padj)
...
outdir <- "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0603_no_Basophil_state_score/output_DESeq2/ml/mouse"
