...
info <- read.xlsx( "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0523_no_Basophil/output_DESeq2/human_auto/unstable_up.xlsx", rowNames = F,colNames = T)
#指定富集分析的物种库
...
KEGG
###GO/KEGG富集柱状图+点状图
outdir <- "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0523_no_Basophil/output_GO_KEGG/output_human_auto/unstable"
...
###############################################stable#########################################################
#载入差异表达数据，只需基因ID(GO,KEGG,GSEA需要)和Log2FoldChange(GSEA需要)即可
info <- read.xlsx( "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0523_no_Basophil/output_DESeq2/human_auto/stable_up.xlsx", rowNames = F,colNames = T)
#指定富集分析的物种库
GO_database <- 'org.Hs.eg.db' #GO分析指定物种，物种缩写索引表详见http://bioconductor.org/packages/release/BiocViews.html#___OrgDb
KEGG_database <- 'hsa' #KEGG分析指定物种，物种缩写索引表详见http://www.genome.jp/kegg/catalog/org_list.html
#gene ID转换
...
KEGG
###GO/KEGG富集柱状图+点状图
outdir <- "/home/lixiangyu/zr/Annotate/ANNOTATE_new/10_downstream/Infer_plaque_type/0523_no_Basophil/output_GO_KEGG/output_human_auto/stable"
p1 <- barplot(GO, split="ONTOLOGY")+facet_grid(ONTOLOGY~., scale="free")#柱状图
p2 <- barplot(KEGG,showCategory = 40,title = 'KEGG Pathway')
ggsave(filename = file.path(outdir, "GO_barplot.png"),plot = p1,width = 10,height = 8,dpi = 300)
ggsave(filename = file.path(outdir, "KEGG_barplot.png"),plot = p2,width = 10,height = 8,dpi = 300)
